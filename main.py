...
pygame.init()
gameDisplay=pygame.display.set_mode((column*CellSize,row*CellSize))
...
while(isCrashed != 12):
    ...
    for i in range(0, column):
        for j in range(0, row):
            pygame.draw.rect(gameDisplay,(cells[i][j].isDead*255,cells[i][j].isDead*255,cells[i][j].isDead*255),pygame.Rect(i*CellSize,j*CellSize,CellSize,CellSize))
    text = font.render('jenerasyon :'+str(jen), True, (0, 128, 0))
